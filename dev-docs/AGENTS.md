...
Odpowiedź nie używa stylu prawnego, formalnych przypisów ani numerów materiałów. Ma formę naturalnego wyjaśnienia przekazywanego przez doświadczonego serwisanta, po którym następują numerowane kroki. Każdy krok opisuje czynność, jej powód i oczekiwany rezultat, a zakończenie obejmuje weryfikację oraz warunki eskalacji. Przy słabym dopasowaniu model wskazuje brakujące dane zamiast wymyślać polecenia, ścieżki lub konfigurację. Prompt zabrania składni Markdown, a warstwa odpowiedzi dodatkowo usuwa gwiazdki oraz znaczniki nagłówków.
