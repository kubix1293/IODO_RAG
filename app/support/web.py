...
def ticket_workbench(ticket_id:uuid.UUID,current=Depends(user)):
    ...
    state=dict(ticket["workflow_state"] or {}); answer=html.escape(state.get("proposed_answer") or "Analiza nie została jeszcze wykonana.")
    sources=[]
    ...
    auto_refresh="setTimeout(()=>location.reload(),3000);" if job and job["status"] in {"queued","running"} else ""
    ...
    <p><a href='/tickets'>← Zgłoszenia</a></p><h1>{html.escape(ticket['program_name'])} · {html.escape(ticket['client_name'])}</h1><p>Status: <strong>{html.escape(str(ticket['status']))}</strong> · zadanie: {html.escape(job_text)}</p>
